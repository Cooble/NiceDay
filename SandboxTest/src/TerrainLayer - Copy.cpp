...
void TerrainLayer::onImGuiRender()
{
	...
	ImGui::Text("Initial Ground Height: %.6f", totalGround);
	ImGui::Text("Current Ground Height: %.6f", currentGround);
	ImGui::Text("Ground Height Diff: %0.6f", totalGround - currentGround);
	ImGui::Text("Water Height: %.6f", currentWater);
...
static void rainDown(Ground& now, Ground& next, float delta)
{
	...
	for (int y = 0; y < now.height; y++)
	{
		for (int x = 0; x < now.width; x++)
		{
			...
			auto d1 = now.water_height[x + y * now.width] + delta * increase;
...
static void flow(Ground& old, Ground& next, float deltaTime)
{
	...
	for (int y = 0; y < height; y++)
	{
		for (int x = 0; x < width; x++)
		{
			...
			// not required since height difference will be 0 anyway because of the clamp
			if (x == 0)
				fluxNew.x = 0;
			if (x == width - 1)
				fluxNew.y = 0;
			...
			// this describes the slowness of water,
			// (we could also calculate many iterations of fixing flux, but then tyhe water would be instant)
			// since totalOutFlux might be even higher than the water level because there is also some influx
	...
	for (int y = 0; y < height; y++)
	{
		for (int x = 0; x < width; x++)
		{
			...
			auto waterColumnCrossArea = meanD * l;
...
static void evaporation(Ground& now, Ground& next, float deltaTime)
{
	...
	for (int y = 0; y < now.height; ++y)
	{
		for (int x = 0; x < now.width; ++x)
		{
			...
			height *= 1 - K_evaporation * deltaTime;
		}
	}
}
...
static void landslide(Ground& now, Ground& next, float deltaTime)
{
	...
	for (int y = 0; y < next.height - 1; ++y)
	{
		for (int x = 0; x < next.width - 1; ++x)
		{
			...
			next.terrain_height[y * now.width + x + 1] -= takeN.x;
			next.terrain_height[(y + 1) * now.width + x] -= takeN.y;
		}
	}
}
...
static void calculateStatistics(Ground& g)
{
	...
	for (int y = 0; y < g.height; y++)
	{
		...
		{
			currentGround += g.terrain_height[x + y * g.width] + g.sediment[x + y * g.width];
			currentWater += g.water_height[y * g.width + x];
			minTerrain = glm::min(minTerrain, g.terrain_height[x + y * g.width]);
			maxTerrain = glm::max(maxTerrain, g.terrain_height[x + y * g.width]);
		}
	}
...
void TerrainLayer::simulate(Ground& now, Ground& next, float delta)
{
	...
	if (e_rain)
		...
		next.water_height = now.water_height;
	...
	if (e_erosion)
		...
		next.sediment = now.sediment;
		next.terrain_height = now.terrain_height;
	}

	if (e_evaporation)
		evaporation(now, next, delta);

	if (e_landslide)
		landslide(now, next, delta);
