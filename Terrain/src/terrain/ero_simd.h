...
#      define SKIP_SIMD_PRIMITIVE 1
#   endif
#   define EROSIMD_H_PRIMITIVE_INCLUDED
#endif
// skip whole file if already included
#if (defined(SKIP_SIMD_PARALLEL) && defined(EROSIMD_PARALLEL_ENABLE)) || (defined(SKIP_SIMD_PRIMITIVE) && !defined(EROSIMD_PARALLEL_ENABLE))
#else


#if EROSIMD_PARALLEL_ENABLE
#include <execution>
#include <algorithm>
#include <thread>
#include <vector>
...
namespace EroParallel {
	...
	inline void ero2_simd(EulerGround& g, Euler::EulerSettings* s)
	{
		...
		// last two elements (w is not multiple of 16)
		auto a = g.total_height[offset + x];
		auto b = g.water_height[offset + x];
		g.total_height[offset + x++] = a + b;

		a = g.total_height[offset + x];
		b = g.water_height[offset + x];
		g.total_height[offset + x++] = a + b;
		END_PARALLELIZE_LOOP
	...
	inline void ero7_simd(EulerGround& g, Euler::EulerSettings* s)
	{
		...
		//PARALLELIZE_LOOP // cannot parallelize due to data dependencies
		for (int y = 1; y < h - 1; ++y)
		{
			const int yw = y * w;
			for (int x = 1; x < w - 16; x += 16)
