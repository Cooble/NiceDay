...
Scene::Scene()
{
	...
	modelShader->bind();
	std::static_pointer_cast<internal::GLShader>(modelShader)->setUniform1i("u_material.diffuse", 0);
	std::static_pointer_cast<internal::GLShader>(modelShader)->setUniform1i("u_material.specular", 1);
	std::static_pointer_cast<internal::GLShader>(modelShader)->setUniform1f("u_material.shines", 64);
...
void Scene::render()
{
	...
	//cam stuff
	{
		std::static_pointer_cast<internal::GLShader>(modelShader)->setUniformVec3f("u_camera_pos", getLookCam()->pos);
		std::static_pointer_cast<internal::GLShader>(modelShader)->setUniformMat4("u_viewMat", view);
		std::static_pointer_cast<internal::GLShader>(modelShader)->setUniformMat4("u_projMat", proj);
	}
	...
	//light stuff
	{
		...
		std::static_pointer_cast<internal::GLShader>(modelShader)->setUniformVec3f("u_light.pos", licht.pos);
		std::static_pointer_cast<internal::GLShader>(modelShader)->setUniformVec3f("u_light.ambient", licht.ambient);
		std::static_pointer_cast<internal::GLShader>(modelShader)->setUniformVec3f("u_light.diffuse", licht.diffuse);
		std::static_pointer_cast<internal::GLShader>(modelShader)->setUniformVec3f("u_light.specular", licht.specular);
		std::static_pointer_cast<internal::GLShader>(modelShader)->setUniform1f("u_light.constant", licht.constant);
		std::static_pointer_cast<internal::GLShader>(modelShader)->setUniform1f("u_light.linear", licht.linear);
		std::static_pointer_cast<internal::GLShader>(modelShader)->setUniform1f("u_light.quadratic", licht.quadratic);
	}
	...
	for (auto& mod : m_models)
	{
		...
		if (model.model->shader != MODEL_SHADER)
			continue;
		std::static_pointer_cast<internal::GLShader>(modelShader)->setUniformMat4("u_worldMat", model.createWorldMatrix());
		std::static_pointer_cast<internal::GLShader>(modelShader)->setUniform1f("u_material.shines", model.shines);
		std::static_pointer_cast<internal::GLShader>(modelShader)->setUniformVec4f("u_material.color", model.color);
...
{
	cubeMapShader->bind();
	std::static_pointer_cast<internal::GLShader>(cubeMapShader)->setUniformMat4("u_projMat", proj);
	...
	for (int i = 0; i < m_models.size(); ++i)
	{
		...
		//draw sky
		std::static_pointer_cast<internal::GLShader>(cubeMapShader)->setUniformMat4("u_viewMat",
