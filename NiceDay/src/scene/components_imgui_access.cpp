...
static void makeDragDropTargetMaterial(Strid& matID)
{
	if (ImGui::BeginDragDropTarget())
	{
		if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("MATERIAL"))
		{
			...
			matID = cp->getID();
			//todo shared ptr problem, we need to change the shared pointer object at location, not the object itself
		}
		ImGui::EndDragDropTarget();
	}
}
...
static std::string fileCombo(const std::string& currentValue, const std::string& sourceFolder, const char** suffix,
                             size_t suffixLength,
                             StringId calledID)
{
	...
	if (calledID() != fileComboLastID)
	{
		...
		if (currentValue.size())
			memcpy(filter.InputBuf, currentValue.c_str(), currentValue.size() + 1);
...
static std::string folderCombo(const std::string& currentValue, const std::string& sourceFolder,
                               const std::function<bool(const std::string& folder)>& isValid, StringId calledID)
{
...
static void drawStringDialog(bool open, const char* popupName, const char* fieldName,
...
static void drawFileDialog(bool open, const char* popupName, const char* fieldName, const std::string& sourceFolder,
                           const char** suffixes, int suffixLength, bool forbidCustomPath,
                           const std::function<void(const char*)>& callBack)
{
...
static void drawFileDialog(bool open, const char* popupName, const char* fieldName, const std::string& sourceFolder,
                           const char* suffix, bool forbidCustomPath, const std::function<void(const char*)>& callBack)
{
	drawFileDialog(open, popupName, fieldName, sourceFolder, &suffix, 1, forbidCustomPath, callBack);
}
}

namespace nd::components_imgui_access {
...
void draw(Entity e, TransformComponent& c)
{
	...
	else
	{
		...
		if (ImGui::DragFloat3("rotation", (float*)&deg, 0.05f, -180.f, 180.f))
			c.rot = glm::radians(deg);
		//refresh only and only if change has occurred, otherwise due to imprecision value would keep changing
	}
	c.recomputeMatrix();
	ImGui::SameLine();
...
static void drawShaderStats(const ShaderPtr& shader)
{
	...
	if (layoutMat)
	{
		...
		{
			ImGui::Text(GTypes::getName(val.type));
			ImGui::NextColumn();
			if (val.arraySize == 1)
		...
		case g_typ::TEXTURE_2D:
			{
				...
				auto newTex = textureCombo(texture ? texture->getFilePath() : "",
				                           element.type == g_typ::TEXTURE_2D ? TextureType::_2D : TextureType::_CUBE_MAP);
	...
	if (ImGui::Begin("Scene", &open, ImGuiWindowFlags_MenuBar))
	{
		...
		if (ImGui::TreeNode("Objects"))
		{
			...
			for (auto ent : tagView)
			{
				...
				comp_util::drawStringDialog(copyFlag, (std::string("entityPop ") + tag.name).c_str(), "Name",
				                            [entity](const char* c)
				                            {
					                            auto ent = windows.scene->createEntity(c);
				...
				ImGui::Checkbox("##enabledee", &enabl);
				ImGui::SameLine();
...
bool drawModelManager(bool clickOnNew)
{
	...
	comp_util::drawFileDialog(clickOnNew, "MaterNew?", "Texture Name", "res/examples/models", suffixes, 4, true,
	                          [](const char* c)
	                          {
		...
		if (ImGui::BeginPopup(c))
		{
			if (!SUtil::endsWith(new_mesh->getName(), ".bin") && ImGui::MenuItem("Export as BIN"))
			{
				MeshDataFactory::writeBinaryFile(
					new_mesh->getName().substr(0, new_mesh->getName().find_last_of('.') + 1) + "bin", *new_mesh->data);
			}
			if (ImGui::MenuItem("Delete"))
			{
	...
	if (ena)
	{
		if (ImGui::Begin("Quantization", &ena))
		{
			...
			else
			{
				...
				if (ImGui::DragFloat("rotation", &deg, 1.f))
					*windows.quantizationRot = glm::radians(deg);
				//refresh only and only if change has occurred, otherwise due to imprecision value would keep changing
			}
			ImGui::SameLine();
			ImGui::Checkbox(useRad ? "Rad" : "Deg", &useRad);
