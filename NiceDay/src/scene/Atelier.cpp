...
void Atelier::snapshot(TexturePtr& photo, MaterialPtr& mat)
{
	...
	//bind other vars
	auto s = std::static_pointer_cast<nd::internal::GLShader>(mat->getShader());
...
void Atelier::snapshot(TexturePtr& photo, MeshPtr& mesh)
{
	...
	//bind other vars
	auto s = std::static_pointer_cast<nd::internal::GLShader>(m_default_material->getShader());
	//if(mod.material->getShader()->getLayout().getLayoutByName("world"))
	glm::mat4 trans(1.f);
