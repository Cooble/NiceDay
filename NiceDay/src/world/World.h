...
public:
	...
	{
		return chunkid.x >= 0 && chunkid.x < getInfo().chunk_width && chunkid.y >= 0 && chunkid.y < getInfo().chunk_height;
	}
	...
	//==============TOTAL METHODS=======================================================================
private:
	//returns block if its loaded or right after generation before anyone else can access it (used for worldgen)
	BlockStruct* getBlockTotal(int x, int y);

	void setBlockTotal(int x, int y, BlockStruct& newBlock);

	void setWallTotal(int x, int y, int wall_id);

	void setBlockWithNotifyTotal(int x, int y, BlockStruct& newBlock);
