...
const int* SUtil::utf8toCodePointsArray(const char* c, int* length)
{
	...
	while ((byte1 = *c++)!=0)
	{
		...
		if ((byte1 & 0b11100000) == 0b11000000) {//starts with 110
			char byte2 = *c++;
		...
		else if ((byte1 & 0b11110000) == 0b11100000) {//starts with 1110
			char byte2 = *c++;
			char byte3 = *c++;
		...
		else if ((byte1 & 0b11111000) == 0b11110000) {//starts with 1110
			char byte2 = *c++;
			char byte3 = *c++;
			char byte4 = *c++;
...
std::u32string SUtil::utf8toCodePoints(const char* c)
{
	//todo use something better than std::vector
	std::u32string out;
	uint8_t byte1;
...
std::string SUtil::u32StringToString(std::u32string_view s)
{
	std::string out;
	for(auto c:s)
		out += (char)c;
