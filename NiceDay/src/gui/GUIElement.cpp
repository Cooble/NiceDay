...
{
   if (isNotSpatial) //lives inside whole parent
	  return true;
...
void GUIElement::onMyEvent(Event& e)
{
   ...
   case Event::EventType::MouseFocusLost:
   {
	  ...
	  if (ev.getPos() != glm::vec2(this->serialID, GUIElement_InvalidNumber))
   ...
   if (!isNotSpatial && e.getEventType() != Event::EventType::MouseMove && e.getEventType() == Event::EventType::MousePress)
	  e.handled = true;
}
...
#define ND_GUI_STYLE_BUILD_2(att) if (style.att.hasValue()) att = style.att;

void GUIElement::applyStyle(GUIStyle& style)
{
   //ND_GUI_STYLE_BUILD_2(textMaterial);
   ND_GUI_STYLE_BUILD_2(dimInherit);
   ND_GUI_STYLE_BUILD_2(isEnabled);
   ND_GUI_STYLE_BUILD_2(renderAngle);
   ND_GUI_STYLE_BUILD_2(isVisible);
   ND_GUI_STYLE_BUILD_2(isNotSpatial);
   ND_GUI_STYLE_BUILD_2(isAlwaysPacked);
   ND_GUI_STYLE_BUILD_2(color);
   ND_GUI_STYLE_BUILD_2(space);
   ND_GUI_STYLE_BUILD_2(width);
   ND_GUI_STYLE_BUILD_2(height);
   ND_GUI_STYLE_BUILD_2(alignment);

   if (style.margin.hasValue())
	  *((glm::vec4*)margin) = style.margin;
   if (style.padding.hasValue())
	  *((glm::vec4*)padding) = style.padding;
}
