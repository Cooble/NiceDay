...
void GUIRenderer::renderCheckBox(BatchRenderer2D& renderer, GUICheckBox& e)
{
	...
	incrementZ();
	renderer.push(glm::translate(glm::mat4(1.0), {
									 m_stackPos.x+42, m_stackPos.y + (e.height - m_font_material->font->lineHeight) / 2,
									 m_z_pos
		}));
	renderer.submitText(e.m_text_mesh, m_font_material);
	renderer.pop();
...
void GUIRenderer::renderSlider(BatchRenderer2D& renderer, GUISlider& e)
{
	...
	renderer.submitColorQuad({ m_stackPos.x+e.padding[GUI_LEFT], m_stackPos.y+e.height/2-backLineHeight/2, m_z_pos }, { width, backLineHeight },
		darkColor);

	incrementZ();
	constexpr float sliderW = 15;
	constexpr float sliderH = 30;
	renderer.submitColorQuad({ m_stackPos.x + e.padding[GUI_LEFT]+ e.getValue()*width-sliderW/2, m_stackPos.y + e.height / 2 - sliderH / 2, m_z_pos }, { sliderW,sliderH },
			brightColor);
}
...
void GUIRenderer::renderTextBox(BatchRenderer2D& renderer, GUITextBox& e)
{
	...
	renderer.submitColorQuad({ m_stackPos.x, m_stackPos.y, m_z_pos }, { e.width, e.height }, (e.hasFocus()||e.hasTotalFocus())?brightColor:lighterColor);
	incrementZ();
	renderer.submitColorQuad({ m_stackPos.x+1, m_stackPos.y+1, m_z_pos }, { e.width-2, e.height-2 }, (e.hasFocus() || e.hasTotalFocus()) ? lightColor : backlightColor);
	incrementZ();
	renderer.push(glm::translate(glm::mat4(1.0), {
									 m_stackPos.x+e.padding[GUI_LEFT], m_stackPos.y + (e.height - m_font_material->font->lineHeight) / 2,
									 m_z_pos
		}));
	renderer.submitText(e.m_text_mesh, m_font_material);
	renderer.pop();
...
{
	if (e.src)
		renderer.submitTextureQuad({ m_stackPos.x,m_stackPos.y,m_z_pos }, { e.width,e.height }, e.src->getUV(), e.src->getTexture());
}
