			...
			if (SUtil::equals(node->name(), "SliderView"))
			{
				...
				if (SUtil::equals(left, "View"))
					parseNode(node->first_node(),
					          vert->isPrimaryLeft() ? vert->getRight()->getFirstChild() : vert->getLeft()->getFirstChild());
				else if (SUtil::equals(left, "VSlider"))
					parseNode(node->first_node(),
					          vert->isPrimaryLeft() ? vert->getLeft()->getFirstChild() : vert->getRight()->getFirstChild());

				if (node->first_node()->next_sibling())
				{
					auto right = node->first_node()->next_sibling()->name();
					if (SUtil::equals(right, "View"))
						parseNode(node->first_node()->next_sibling(),
						          vert->isPrimaryLeft()
							          ? vert->getRight()->getFirstChild()
							          : vert->getLeft()->getFirstChild());
