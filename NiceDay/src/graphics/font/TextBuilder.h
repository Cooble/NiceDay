...
public:
	...
	void setChar(int index, float x, float y, float x1, float y1, uint32_t color, uint32_t borderColor,
	             const Character& ch);

	void setChar(int index, float x, float y, float x1, float y1, float u, float v, float u1, float v1, uint32_t color,
	             uint32_t borderColor, const Character& ch);
...
public:
	...
	static void convertToLines(const std::string& text, const Font& font, int maxLineWidth,
	                           std::vector<std::u32string>& lines);

	//updates mesh with data
	//return false if mesh is too small to fit all chars in
	//clipRect {minX,minY,maxX,maxY}
	static bool buildMesh(const std::vector<std::u32string>& lines, const Font& font, TextMesh& mesh,
	                      int alignment = ALIGN_LEFT,
	                      glm::vec<4, int> clipRect = {-10000, -10000, 20000, 20000}, CursorProp* cursor = nullptr);

	static bool buildMesh(const std::string& text, int maxLineWidth, const Font& font, TextMesh& mesh,
	                      int alignment = ALIGN_LEFT,
	                      glm::vec<4, int> clipRect = {-10000, -10000, 20000, 20000}, CursorProp* cursor = nullptr)
	{
		std::vector<std::u32string> lines;
